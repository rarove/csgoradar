  ...
  return (
    <div className="w-screen h-screen flex flex-col overflow-hidden" style={{ background: `radial-gradient(50% 50% at 50% 50%, rgba(20, 40, 55, 0.95) 0%, rgba(7, 20, 30, 0.95) 100%)`, backdropFilter: `blur(7.5px)` }}>
      <div className="w-full h-full flex flex-col justify-center overflow-hidden relative p-1 lg:p-2">
        ...
        <div className="sm:hidden grid grid-cols-3 gap-1 mt-2 px-1">
          {playerArray.map(p=>(
            <div key={p.m_idx} className="flex items-center gap-1 bg-black/40 rounded-md px-1.5 py-1 text-[10px] border border-white/10 overflow-hidden">
              <img src={`./assets/characters/${p.m_model_name||(p.m_team==2?'tm_phoenix':'ctm_sas')}.png`} className="w-6 h-6 object-contain shrink-0 bg-black/20 rounded" onError={e=>e.currentTarget.style.display='none'} />
              <div className="flex flex-col leading-none min-w-0 flex-1">
                <span className="font-medium truncate text-[10px]">{p.m_name}{p.m_is_local?' ★':''}</span>
                <span className="opacity-60 text-[8px]">{p.m_health}hp • ${p.m_money||0}</span>
              </div>
            </div>
          ))}
        </div>
