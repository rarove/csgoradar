...
const std::string c_entity_instance::get_schema_class_name()
{
	...
	auto try_read = [&](uintptr_t off1, uintptr_t off2) -> std::string {
		auto u1 = m_memory->read_t<uintptr_t>(class_info + off1);
		if (!u1) return {};
		auto u2 = m_memory->read_t<uintptr_t>(u1 + off2);
		if (!u2) return {};
		return m_memory->read_t<std::string>(u2);
	};
	auto name = try_read(0x58, 0x08);
	if (!name.empty()) return name;
	name = try_read(0x48, 0x08);
	return name;
}
...
{
	static auto cached = []() -> void* {
		auto try_rip = [](const char* pat) -> std::optional<c_address> {
			auto a = m_memory->find_pattern(CLIENT_DLL, pat);
			if (a.has_value()) return a->rip();
			return std::nullopt;
		};
		if (auto a = try_rip(GET_LOCAL_PLAYER_CONTROLLER); a.has_value()) return a->as<void*>();
		if (auto a = try_rip(GET_LOCAL_PLAYER_CONTROLLER_ALT); a.has_value()) return a->as<void*>();
		auto [base, sz] = m_memory->get_module_info(CLIENT_DLL);
		if (base.has_value()) return m_memory->read_t<void*>(base.value() + offsets::dwLocalPlayerController);
		return nullptr;
	}();
	if (!cached) return {};
	return m_memory->read_t<c_cs_player_controller*>(cached);
}
